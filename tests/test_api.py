#!/usr/bin/env python3
"""Comprehensive test suite for Task Dashboard API."""
...
class TestTaskCRUDOperations:
    ...
    def test_create_task_invalid_empty_title(self):
    ...
    def test_get_all_tasks_with_data(self, sample_task):
    ...
    def test_update_task_partial(self, sample_task):
...
class TestTaskStatusUpdate:
    ...
    def test_update_task_status(self, sample_task):
        """Test updating task status via PATCH endpoint."""
        task_id = sample_task["id"]
        status_data = {"status": "done"}
        response = client.patch(f"/tasks/{task_id}/status", json=status_data)
        assert response.status_code == 200
    ...
    def test_update_task_status_not_found(self):
        ...
        response = client.patch("/tasks/999/status", json=status_data)
        assert response.status_code == 404
...
class TestTaskFiltering:
    ...
    def test_filter_by_status(self, multiple_tasks):
        """Test filtering tasks by status."""
        response = client.get("/tasks?status=todo")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 5  # All tasks have status "todo" due to API design
        assert all(task["status"] == "todo" for task in data)
    
    def test_filter_by_priority(self, multiple_tasks):
    ...
    def test_filter_by_status_and_priority(self, multiple_tasks):
    ...
    def test_search_case_insensitive(self, multiple_tasks):
        """Test case-insensitive search."""
        response = client.get("/tasks?search=REGULAR")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
        assert data[0]["title"] == "Regular Task"
    
    def test_search_in_description(self, multiple_tasks):
    ...
    def test_no_results_filtering(self, multiple_tasks):
        """Test filtering that returns no results."""
        response = client.get("/tasks?status=todo&priority=invalid_priority")
        assert response.status_code == 200
        assert response.json() == []
